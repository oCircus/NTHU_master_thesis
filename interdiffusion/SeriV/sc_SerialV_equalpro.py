...
class SCInterdiffusionSerialV:
    ...
    def find_gradient_layer(
        ...
        if left_after_con - left_before_con >= 0.05:
            left_idz_layer -= 1
        if right_before_con - right_after_con >= 0.05:
            right_idz_layer += 1
    ...
    def save_data(
        ...
        dirname = f"sc_serial_dynamic_evolution_equalpro_data_size{parameter[-1]}_numofvacancy{parameter[-3]}_{parameter[1]}"
        # file_dir = r"/work1/u3284480/"
        dirpath = os.path.join(
            rf"E:\Diffusion\interdiffusion\serialresult\0.05\scLattice", dirname
        )
